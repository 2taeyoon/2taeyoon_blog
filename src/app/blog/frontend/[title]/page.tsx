...
import FrontStudyContent from "@/app/blog/frontend/[title]/FrontStudy";
...
export async function generateMetadata({ params }: { params: Promise<{ title: string }> }) { // 동적 경로 매개변수인 params로 URL 가져옴
  ...
  const frontStudyFind = frontendData.cards.find((item) => item.title === replaceTitle); // JSON 데이터와 title을 비교하여 알맞는 데이터 찾음
  ...
  return {
    ...
    openGraph: {
      title: frontStudyFind.title,
      description: frontStudyFind.subTitle,
      url: `https://www.2taeyoon.com/blog/frontend/${decodedTitle}`,
			images: [
				{
